...
namespace Domain.Aggregates
{
    public class Payment : AggregateRoot<PaymentValidator>
    {
        ...
        public void Handle(Command.SellerConfirmPayment cmd)
            => RaiseEvent<DomainEvent.PaymentSellerConnfirm>(version => new(Id, RestaurantId, Total, true, version));

        protected override void Apply(IDomainEvent @event)
            => When(@event as dynamic);

        public void When(DomainEvent.PaymentRequest @event)
            => (Id, OrderId, RestaurantId, Total, PaymentMethod, TransactionId, PaymentStatus, SellerConfirm, _, _) = @event;
